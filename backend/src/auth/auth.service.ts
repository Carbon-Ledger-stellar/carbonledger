...
export class AuthService {
  ...
  async verifySignatureAndLogin(
    ...
    try {
      ...
      // Determine if this is a new signup vs returning login
      const isNewUser = user.createdAt.getTime() === user.updatedAt.getTime();

      // Analytics: identify user and track event
      this.analytics.identify(user.publicKey, { role: user.role, createdAt: user.createdAt });
      if (isNewUser) {
        this.analytics.track(user.publicKey, AnalyticsEvent.USER_SIGNED_UP, { role: user.role });
      } else {
        this.analytics.track(user.publicKey, AnalyticsEvent.USER_LOGGED_IN, { role: user.role });
      }
